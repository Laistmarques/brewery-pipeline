...
def test_gold_transformer_aggregates_by_type_country_state_city(spark):
    ...
    df = spark.createDataFrame([
        # Mesmo grupo (micro, BR, SP, Sao Paulo) com 2 ids diferentes
        Row(id="1", brewery_type="micro", country="BR", state="SP", city="Sao Paulo"),
        Row(id="2", brewery_type="micro", country="BR", state="SP", city="Sao Paulo"),

        # Mesmo grupo mas id repetido (não deve contar 2x)
        Row(id="2", brewery_type="micro", country="BR", state="SP", city="Sao Paulo"),

        # Outro grupo com 1 id
        Row(id="3", brewery_type="brewpub", country="BR", state="SP", city="Campinas"),
    ])
